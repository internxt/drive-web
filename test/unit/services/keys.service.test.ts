...
import { getKeys, decryptPrivateKey, assertValidateKeys, KeysDoNotMatchError } from '../../../src/app/crypto/services/keys.service';
  ...
  it('Should not update private key if encryption & encoding is fine', async () => {
    ...
    await expect(
      assertValidateKeys(
        ...
        Buffer.from(keys_different.publicKeyArmored,'base64').toString(),
      )
