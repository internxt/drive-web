...
const BaseDialog = ({
  ...
  return (
    <div className={`${open ? 'flex' : 'hidden'} fixed z-50 flex align-middle justify-center w-full h-full`}>
      {/* BACKGROUND */}
      <div className={`${open ? 'flex' : 'hidden'} z-40 absolute opacity-80 bg-m-neutral-100 w-full h-full`}></div>

      {/* PANEL */}
      <div className={`base-dialog-panel ${open ? 'flex flex-col' : 'hidden'} relative z-50 rounded-lg pt-8 bg-white text-xs overflow-hidden`}>
