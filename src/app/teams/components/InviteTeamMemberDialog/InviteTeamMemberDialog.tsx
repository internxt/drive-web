...
const InviteTeamMemberDialog = ({ team }: InviteTeamMemberDialogProps) => {
  ...
  const deleteMembers = async (memberToDelete: InfoInvitationsMembers) => {
    ...
    try {
      ...
      notificationsService.show({ text: t('success.deletedTeamMember', { resource }), type: ToastType.Success });
    } catch (err: unknown) {
      notificationsService.show({ text: t('error.deleteTeamMember'), type: ToastType.Error });
    }
  };
