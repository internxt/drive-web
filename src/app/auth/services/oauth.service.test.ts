...
describe('OAuth Service', () => {
  ...
  describe('sendAuthError method', () => {
    const mockErrorMessage = 'Authentication failed';

    it('should send error message to the opener window when opener exists and origin is allowed', () => {
      Object.defineProperty(window, 'opener', {
        value: mockOpener,
        writable: true,
        configurable: true,
      ...
      const result = oauthService.sendAuthError(mockErrorMessage);
    ...
    it('should return false when target origin is not in the allowed origins list', () => {
      const disallowedOpener = {
        postMessage: vi.fn(),
        location: {
          origin: 'https://malicious-site.com',
    ...
    it('should use document.referrer as fallback origin when opener.location is not accessible', () => {
      const crossOriginOpener = {
        postMessage: vi.fn(),
        get location() {
          throw new Error('Cross-origin access denied');
