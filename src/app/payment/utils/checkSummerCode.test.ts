...
import { isSummerThemeAvailable } from './checkSummerCode';
import localStorageService from 'app/core/services/local-storage.service';
import paymentService from '../services/payment.service';
import errorService from 'app/core/services/error.service';
import { STORAGE_KEYS } from 'app/core/services/storage-keys';

const COUPON = 'SUMMER80';

describe('isSummerThemeAvailable', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('returns true if localStorage has the key set to "true"', async () => {
    ...
    const result = await isSummerThemeAvailable();

    expect(result).toBe(true);
    expect(getSpy).toHaveBeenCalledWith(STORAGE_KEYS.THEMES.SUMMER_THEME_AVAILABLE_LOCAL_STORAGE_KEY);
  });
  ...
  it('returns true, calls onSuccess, and sets localStorage if the coupon is used', async () => {
    ...
    const result = await isSummerThemeAvailable(onSuccess);

    expect(result).toBe(true);
    expect(onSuccess).toHaveBeenCalled();
    expect(couponSpy).toHaveBeenCalledWith(COUPON);
    expect(setSpy).toHaveBeenCalledWith(STORAGE_KEYS.THEMES.SUMMER_THEME_AVAILABLE_LOCAL_STORAGE_KEY, 'true');
  });
