  ...
  return (
    <Modal maxWidth="max-w-md" isOpen={isOpen} onClose={onClose}>
      <div className="flex flex-col space-y-5">
        <p className="text-2xl font-medium text-gray-100">{get('drive.clearTrash.title')}</p>
        <p className="text-lg text-gray-80">{get('drive.clearTrash.advice')}</p>
        ...
        <div className="flex flex-row items-center justify-end space-x-2">
          ...
          <Button disabled={isLoading} variant="accent" onClick={onAccept}>
            {isLoading ? get('drive.clearTrash.progress') : get('drive.clearTrash.accept')}
          </Button>
        </div>
      </div>
    </Modal>
