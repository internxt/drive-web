...
/**
 * Applies the chosen collision resolution to the given items. Moved items are removed from the
 * pending-deletion list once handled; skipped items are left untouched.
 */
export const resolveCollision = async (
  { operationType, operation, items, existingItems, destinationUuid }: ResolveCollisionParams,
  context: NameCollisionContext,
): Promise<void> => {
  if (operation === 'skip') return;

  if (operationType === 'move') {
    const movedItems = items as DriveItemData[];
    if (operation === 'keep') {
      await keepAndMoveItems(movedItems, destinationUuid, context);
    } else {
      await replaceAndMoveItems(getCollisionPairs(movedItems, existingItems), destinationUuid, context);
    }
    context.dispatch(storageActions.popItemsToDelete(movedItems));
    return;
  }

  const uploadedItems = items as (IRoot | File)[];
  if (operation === 'keep') {
    await keepAndUploadItems(uploadedItems, destinationUuid, context);
  } else {
    await replaceAndUploadItems(getCollisionPairs(uploadedItems, existingItems), destinationUuid, context);
  }
};
