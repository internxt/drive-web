...
describe('NetworkFacade', () => {
  ...
  describe('Downloading a file with bucket key', () => {
    const bucketKey = Buffer.alloc(32, 0x02);

    function mockDownloadFileWithBucketKey(fileInfoOverride: object = {}) {
      (downloadFileWithBucketKey as any).mockImplementation(
        async (_fId, _bId, _bucketKey, _net, _crypto, _buf, downloadCb, decryptCb) => {
          await downloadCb([{ url: downloadUrl }], { size: 100, ...fileInfoOverride });
          await decryptCb('AES256CTR', fakeKey, fakeIv, 100);
        },
      );
    ...
    test('When the download resolves, downloadFileWithBucketKey is called with the bucket key, bucketId and fileId', async () => {
      ...
      expect(downloadFileWithBucketKey).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          ...
          generateFileKeyFromBucketKey: expect.any(Function),
          computeHmac: expect.any(Function),
        }),
        Buffer.from,
        expect.any(Function),
        expect.any(Function),
    ...
    test('When the computed HMAC is correct, resolves without error', async () => {
      const expectedHmac = 'matching-hmac-value';
      mockDownloadFileWithBucketKey({ hmac: { type: 'sha512', value: expectedHmac } });
      vi.mocked(getFileHmacFromShardHashes).mockResolvedValue(expectedHmac);
    ...
    test('When the computed HMAC is not correct, throws error', async () => {
      mockDownloadFileWithBucketKey({ hmac: { type: 'sha512', value: 'expected-hmac' } });
      vi.mocked(getFileHmacFromShardHashes).mockResolvedValue('different-hmac');
    ...
    test('When computing the HMAC, getFileHmacFromShardHashes receives the shard hashes collected during download', async () => {
      mockDownloadFileWithBucketKey({ hmac: { type: 'sha512', value: 'some-hmac' } });
      vi.mocked(getFileHmacFromShardHashes).mockResolvedValue('some-hmac');
    ...
    test('When options.key is provided, it overrides the SDK-provided keyx', async () => {
      const overrideKey = Buffer.alloc(32, 0x09);
      mockDownloadFileWithBucketKey({ hmac: { type: 'sha512', value: 'matches-override' } });
      vi.mocked(getFileHmacFromShardHashes).mockResolvedValue('matches-override');
    ...
    test('When a token is provided in options, it is forwarded to downloadFileWithBucketKey', async () => {
      mockDownloadFileWithBucketKey();
      vi.mocked(getFileHmacFromShardHashes).mockResolvedValue('any-hmac');
    ...
    test('When no token is provided, downloadFileWithBucketKey receives undefined as the options argument', async () => {
      mockDownloadFileWithBucketKey();
      vi.mocked(getFileHmacFromShardHashes).mockResolvedValue('any-hmac');
    ...
    test('When the download is aborted, then an error is thrown', async () => {
      const abortController = new AbortController();
      let callCount = 0;

      (downloadFileWithBucketKey as any).mockImplementation(
        async (_fId, _bId, _bk, _net, _crypto, _buf, downloadCb) => {
          await downloadCb([{ url: downloadUrl }, { url: downloadUrl }], { size: 100 });
        },
      );
