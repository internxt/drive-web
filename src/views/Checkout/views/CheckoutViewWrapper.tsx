...
import BfCheckoutView from 'views/BfCheckout/views/BfCheckoutView';
import { getBfCheckoutVariant } from 'views/BfCheckout/constants';
import { useBfStripeAppearance } from 'views/BfCheckout/hooks/useBfStripeAppearance';

const CheckoutViewWrapper = () => {
  const { translate } = useTranslationContext();
  const { checkoutTheme } = useThemeContext();
  const isBfCheckoutRoute = navigationService.isCurrentPath(AppView.BfCheckout);
  ...
  const { isCheckoutReady, stripeElementsOptions, availableCryptoCurrencies, stripeSdk } = useInitializeCheckout({
    user,
    price: selectedPlan,
    checkoutTheme: isBfCheckoutRoute ? 'dark' : (checkoutTheme ?? 'light'),
    translate,
  });

  const bfStripeElementsOptions = useBfStripeAppearance(
    stripeElementsOptions,
    isBfCheckoutRoute ? selectedPlan : undefined,
  );
  const elementsOptions = isBfCheckoutRoute ? bfStripeElementsOptions : stripeElementsOptions;
  const bfVariant = useMemo(
    () => getBfCheckoutVariant(new URLSearchParams(globalThis.location.search).get('variant')),
    [],
  );
