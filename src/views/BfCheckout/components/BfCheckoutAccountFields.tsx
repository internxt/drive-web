  ...
  return (
    <div role="none" onKeyDown={preventSubmitOnEnter} className="flex flex-col gap-4">
      <div className="flex flex-col gap-1.5">
        <FieldLabel>{translate('bfCheckout.form.emailLabel')}</FieldLabel>
        <input
          type="email"
          autoComplete="email"
          placeholder={translate('bfCheckout.form.emailPlaceholder')}
      ...
      <div className="flex flex-col gap-1.5">
        <FieldLabel>
          {isSignUp ? translate('bfCheckout.form.createPasswordLabel') : translate('bfCheckout.form.passwordLabel')}
        </FieldLabel>
        <div className="relative">
          <input
            type={isPasswordVisible ? 'text' : 'password'}
            autoComplete={isSignUp ? 'new-password' : 'current-password'}
            placeholder={translate('bfCheckout.form.passwordPlaceholder')}
            className={`${BF_CHECKOUT_INPUT_CLASSNAME} pr-12`}
            {...register('password', { required: true, maxLength: MAX_PASSWORD_LENGTH })}
          />
          <button
            type="button"
            aria-label={translate('bfCheckout.form.togglePasswordVisibility')}
      ...
      {(authError || errors.email || errors.password) && (
        <p id="authError" className="text-sm text-red">
          {authError ?? translate('bfCheckout.form.incompleteDetails')}
        </p>
      )}
      ...
      <div className="flex flex-row items-start gap-3 rounded-xl border border-[#1E5B36] bg-[#0E2B1B] p-4">
        ...
        <div className="flex flex-col">
          <p className="text-sm font-semibold text-[#3BD16F]">{translate('bfCheckout.form.encryptedTitle')}</p>
          <p className="text-sm text-[#A7B4C8]">{translate('bfCheckout.form.encryptedDescription')}</p>
        </div>
      </div>
