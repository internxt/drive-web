...
export const BfCheckoutHero = ({ planSummary, creativeImage }: BfCheckoutHeroProps): JSX.Element => {
  ...
  const offerPillLabel = discountPercent
    ? translate('checkout.bfCheckout.hero.offerPill', { percent: discountPercent })
    : translate('checkout.bfCheckout.hero.offerPillNoDiscount');

  const offerPointTitle = discountPercent
    ? translate('checkout.bfCheckout.hero.points.offer.title', { percent: discountPercent })
    : translate('checkout.bfCheckout.hero.points.offer.titleNoDiscount');
    ...
    <section className="flex w-full flex-col gap-8">
      ...
      <div className="flex flex-col gap-4">
        <h1 className="text-4xl font-bold leading-tight text-white sm:text-5xl">
          {translate('checkout.bfCheckout.hero.title')}{' '}
          <span className="text-[#4D9BFF]">{translate('checkout.bfCheckout.hero.titleHighlight')}</span>
        </h1>
        <p className="text-xl leading-snug text-[#C4CEDD]">
          {translate('checkout.bfCheckout.hero.subtitle', { storage, price: priceLabel })}
        </p>
      </div>
      ...
      <ul className="flex flex-col gap-6">
        <SellingPoint
          icon={LockIcon}
          title={translate('checkout.bfCheckout.hero.points.privacy.title')}
          description={translate('checkout.bfCheckout.hero.points.privacy.description')}
        />
        <SellingPoint
          icon={FolderSimpleIcon}
          title={translate('checkout.bfCheckout.hero.points.storage.title')}
          description={translate('checkout.bfCheckout.hero.points.storage.description')}
        />
        <SellingPoint
          icon={TagIcon}
          title={offerPointTitle}
          description={translate('checkout.bfCheckout.hero.points.offer.description', { storage, price: priceLabel })}
        />
      </ul>

      <img
        src={creativeImage}
        alt={translate('checkout.bfCheckout.hero.imageAlt')}
        className="w-full rounded-2xl border border-[#202D45] object-cover"
        loading="eager"
      />

      <div className="grid grid-cols-2 gap-3 xl:grid-cols-4">
        <FooterBadge icon={GlobeHemisphereWestIcon} label={translate('checkout.bfCheckout.hero.badges.basedInEu')} />
        <FooterBadge icon={CodeIcon} label={translate('checkout.bfCheckout.hero.badges.openSource')} />
        <FooterBadge icon={ShieldCheckIcon} label={translate('checkout.bfCheckout.hero.badges.audited')} />
        <FooterBadge icon={InfinityIcon} label={translate('checkout.bfCheckout.hero.badges.privacyPriority')} />
      </div>
    </section>
  );
};
