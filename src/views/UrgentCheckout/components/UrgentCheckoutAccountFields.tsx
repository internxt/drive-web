...
interface UrgentCheckoutAccountFieldsProps {
...
export const UrgentCheckoutAccountFields = ({
  authMethod,
  errors,
  register,
  userData,
  ...
  return (
    <div role="none" onKeyDown={preventSubmitOnEnter} className="flex flex-col gap-4">
      <div className="flex flex-col gap-1.5">
        <FieldLabel>{translate('urgentCheckout.form.emailLabel')}</FieldLabel>
        <input
          type="email"
          autoComplete="email"
          placeholder={translate('urgentCheckout.form.emailPlaceholder')}
          className={URGENT_CHECKOUT_INPUT_CLASSNAME}
          {...register('email', {
            required: true,
      ...
      <div className="flex flex-row items-start gap-3 rounded-xl border border-[#1E5B36] bg-[#0E2B1B] p-4">
        ...
        <div className="flex flex-col">
          <p className="text-sm font-semibold text-[#3BD16F]">{translate('urgentCheckout.form.encryptedTitle')}</p>
          <p className="text-sm text-[#A7B4C8]">{translate('urgentCheckout.form.encryptedDescription')}</p>
        </div>
      </div>
