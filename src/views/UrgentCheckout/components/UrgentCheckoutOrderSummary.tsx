...
interface UrgentCheckoutOrderSummaryProps {
...
export const UrgentCheckoutOrderSummary = ({ planSummary }: UrgentCheckoutOrderSummaryProps): JSX.Element => {
  ...
  return (
    <div className="flex flex-col gap-4">
      {hasDiscount && (
        <div className="flex w-fit flex-row items-center gap-1.5 rounded-full border border-[#1E5B36] bg-[#0E2B1B] px-3 py-1.5">
          ...
          <p className="text-sm font-semibold text-[#3BD16F]">
            {translate('urgentCheckout.summary.discountApplied', { percent: discountPercent })}
          </p>
        </div>
      )}
      ...
      <div className="flex flex-col gap-3">
        ...
        {hasTaxes && (
          <SummaryRow
            label={translate('urgentCheckout.summary.estimatedTax')}
