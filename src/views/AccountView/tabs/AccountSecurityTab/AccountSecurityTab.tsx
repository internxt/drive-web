  ...
  return (
    <div className="account-security-tab flex flex-col items-center justify-center">
      ...
      <p className="account_config_description">
        Two-Factor Authentication provides an extra layer of security for your Internxt account by requiring a second
        step of verification when you sign in. In addition to your password, you'll also need a code generated by Authy,
        Google Authenticator or a similar on your phone. Follow the steps below to enable 2FA on your account.
      </p>
      ...
      {!isLoading ? (
        <Fragment>
          <div
            className={`${
              has2FA ? 'hidden' : 'flex'
            } justify-between my-8 security-buttons_container security-container`}
          >
            <button
              onClick={() => pickStep(1)}
              className={`${currentStep === 1 ? 'security-button security-active_button' : 'security-button'}`}
            >
            ...
            <button
              onClick={() => pickStep(2)}
              className={`${currentStep === 2 ? 'security-button security-active_button' : 'security-button'}`}
            >
            ...
            <button
              onClick={() => pickStep(3)}
              className={`${currentStep === 3 ? 'security-button security-active_button' : 'security-button'} ${
                currentStep - 3 < -1 && 'cursor-default'
              }`}
            >
            ...
            <button
              onClick={() => pickStep(4)}
              className={`${currentStep === 4 ? 'security-button security-active_button' : 'security-button'} ${
                currentStep - 4 < -1 && 'cursor-default'
              }`}
            >
