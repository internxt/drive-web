...
beforeEach(() => {
  ...
  localStorage.setItem(LocalStorageItem.UserID, mockUserID);
...
describe('Testing the local storage service', () => {
  ...
  describe('Backup key acknowledgment', () => {
    const seenAtKey = `backup_key_seen_at_${mockUserID}`;
    const acknowledgedKey = `backup_key_acknowledged_at_${mockUserID}`;
